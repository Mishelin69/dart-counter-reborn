...
static void flush_callback(lv_disp_t *drv, const lv_area_t *area, uint8_t *color_map)
{
    esp_lcd_panel_handle_t panel_handle = (esp_lcd_panel_handle_t) lv_display_get_user_data(drv); // Get the panel handle from driver user data
    const int offsetx1 = area->x1; // Start X coordinate of the area to flush
    const int offsetx2 = area->x2; // End X coordinate of the area to flush
    const int offsety1 = area->y1; // Start Y coordinate of the area to flush
    const int offsety2 = area->y2; // End Y coordinate of the area to flush
...
static lv_disp_t *display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    static lv_display_t* disp_drv = lv_display_create(800, 480);          // Contains LCD panel handle and callback functions
    ...
    lv_display_set_buffers(disp_drv, buf1, buf2, buffer_size * sizeof(lv_color_t), LV_DISPLAY_RENDER_MODE_DIRECT);
...
#endif
    lv_display_set_flush_cb(disp_drv, flush_callback);
...
#endif
    return disp_drv; // Register the display driver
}

static void touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    esp_lcd_touch_handle_t tp = (esp_lcd_touch_handle_t) lv_indev_get_user_data(indev_drv); // Get touchpad handle from user data
...
static lv_indev_t *indev_init(esp_lcd_touch_handle_t tp)
{
    ...
    static lv_indev_t* indev_drv_tp = lv_indev_create(); // Static input device driver
    ...
    return indev_drv_tp; // Register the input device driver
}
